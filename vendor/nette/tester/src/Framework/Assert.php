...
class Assert
{
	...
	public static function error($function, $expectedType, $expectedMessage = NULL)
	{
		...
		foreach ($expected as & $item) {
			...
			if (is_int($expectedType)) {
				...
				throw new \Exception('Error4xx type must be E_* constant.');
			}
		}
		...
		if ($expected) {
			self::fail('Error4xx was expected, but was not generated');
		}
	}
	...
	public static function isMatching($pattern, $actual)
	{
		...
		if ($res === FALSE || preg_last_error()) {
			throw new \Exception('Error4xx while executing regular expression. (PREG Error4xx Code ' . preg_last_error() . ')');
		}
		return (bool) $res;
	}
